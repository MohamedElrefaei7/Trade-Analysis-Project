...
def scratch_db(monkeypatch):
    ...
    # schema.sql), so minimal stand-ins (positions needs a real vessel_id
    # column now too, for 0004's index target) are required before
    # run_migrations() gets there.
    ...
    with setup_conn.cursor() as cur:
        ...
        cur.execute("CREATE TABLE positions (vessel_id UUID, ts TIMESTAMPTZ NOT NULL)")
    setup_conn.close()
